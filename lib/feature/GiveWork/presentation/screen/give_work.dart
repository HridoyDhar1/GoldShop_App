...
import 'package:goldshop/feature/workerList/data/model/work_model.dart';
...
class GiveWorkScreen extends StatefulWidget {
  const GiveWorkScreen({super.key});

  static const String name = "give_work";

  @override
  State<GiveWorkScreen> createState() => _GiveWorkScreenState();
}

class _GiveWorkScreenState extends State<GiveWorkScreen> {
  ...
  void _saveAndShowDetails() async {
    ...
    try {
      ...
      await FirebaseFirestore.instance.collection('GiveWork').add(
        workModel.toMap(), // Ensure you call `toMap()` on the workModel
      );
      ...
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => WorkingListScreen(workList: workingList),
        ),
      );

      // Navigate to WorkingDetailsScreen after a short delay
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text("Give Work", style: TextStyle(fontSize: 20)),
      ),
      body: SafeArea(
  ...
  void dispose() {
    ...
    for (var controller in nameControllers)
      controller.dispose();
    for (var controller in carretControllers)
      controller.dispose();
    for (var controller in voriControllers)
      controller.dispose();
    for (var controller in anaControllers)
      controller.dispose();
    for (var controller in rothiControllers)
      controller.dispose();
